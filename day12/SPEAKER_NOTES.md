...
안녕하세요. **문제5 「내일 날씨는 맑음」**, `day12`의 **`mars_weather_summary.py`** 발표입니다.

스토리는 간단합니다. 화성 이동 경로는 정해졌는데, **이동 날짜에 모래 폭풍이 겹치면 위험**합니다. 그래서 미션 컴퓨터에 백업된 **`mars_weathers_data.csv`** 를 MySQL에 넣고, **이동해도 되는지** 확인한 뒤 **`mars_weather_summary.png`** 로 결과를 남깁니다.

쓴 파일은 **`mars_weathers_data.csv`**(과제 데이터), **`mars_weather_summary.py`**(메인), **`create_mars_weather.sql`**(테이블 DDL), **`db_config.local.py`**(MySQL 비밀번호) 네 가지면 됩니다.

이제 **코드가 어떻게 돌아가는지** 순서대로 설명하겠습니다.
...
과제에서 「내용을 **확인**하라」고 해서, `csv.reader`로 파일을 열고 **한 줄씩 `print`** 합니다.
...
파일이 없으면 `FileNotFoundError`를 잡아서 오류 메시지를 냅니다. **표준 라이브러리만** 사용합니다.

---

### 3. `_parse_csv_rows` — DB에 넣을 형태로 변환

여기가 데이터 전처리입니다. 세 가지를 처리합니다.
...
CSV에 `weather_id`가 있어도, DB는 **AUTO_INCREMENT**라서 INSERT할 때는 **`mars_date`, `temp`, `storm`만** 딕셔너리로 만듭니다. 1000건이 나옵니다.
...
matplotlib은 쓸 수 없어서 **`struct` + `zlib`** 로 PNG를 직접 만듭니다.  
